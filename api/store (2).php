...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cart-badge {
    ...
    background: rgba(106,191,63,0.15); border: 1px solid rgba(106,191,63,0.3);
    color: var(--white); padding: 6px 14px; border-radius: 20px; font-size: 0.85rem; cursor: pointer;
  }
  .cart-badge .num {
  ...
  .login-box {
    ...
    box-shadow: 0 10px 40px rgba(0,0,0,0.08); animation: fadeUp 0.5s ease both;
  }
  ...
  .btn-green { width: 100%; padding: 13px; background: var(--lime); color: var(--dark); border: none; border-radius: 10px; font-family: 'Syne', sans-serif; font-size: 1rem; font-weight: 700; cursor: pointer; transition: background 0.2s; }
  ...
  .hint { text-align: center; margin-top: 14px; font-size: 0.8rem; color: var(--gray); }
  ...
  .qty-input:focus { border-color: var(--lime); }
  .btn-add { flex: 1; padding: 7px 10px; background: var(--lime); color: var(--dark); border: none; border-radius: 8px; font-family: 'Inter', sans-serif; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: background 0.2s; display: flex; align-items: center; justify-content: center; gap: 5px; }
  ...
  .ci-img { width: 44px; height: 44px; border-radius: 10px; background: var(--light); font-size: 1.8rem; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .ci-info { flex: 1; }
  ...
  .btn-del { background: none; border: none; color: #c0c0c0; font-size: 1rem; cursor: pointer; padding: 4px 6px; border-radius: 6px; transition: color 0.2s, background 0.2s; text-decoration: none; }
  ...
  .btn-order { display: block; width: 100%; margin-top: 14px; padding: 13px; background: var(--dark); color: var(--white); border: none; border-radius: 10px; font-family: 'Syne', sans-serif; font-size: 0.95rem; font-weight: 700; cursor: pointer; text-align: center; text-decoration: none; transition: background 0.2s; }
  .btn-order:hover { background: var(--green); }
  .btn-vider { display: block; text-align: center; margin-top: 10px; font-size: 0.8rem; color: var(--gray); text-decoration: none; }
...
if (!empty($_GET['vider'])) { $panier->vider(); }
...
<header>
  ...
  <nav>
    <a href="store.php" class="active">Accueil</a>
...
<div class="login-wrap">
  <div class="login-box">
    ...
    <form method="POST">
      ...
      <input class="inp" type="text" name="login" placeholder="admin" value="<?= htmlspecialchars($_POST['login'] ?? '') ?>" required autofocus>
    ...
    <div class="cart-box">
      ...
      <?php else: ?>
        ...
        <div class="cart-item">
          ...
          <a href="store.php?del=<?= $f->id ?>" class="btn-del" title="Supprimer">
        ...
        <a href="store.php?vider=1" class="btn-vider">🗑 Vider le panier</a>
      <?php endif; ?>
    </div>
  </div>
